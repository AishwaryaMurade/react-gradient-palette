      ...
      <div style={styles.tabcontent}>
        ...
        <div style={styles.gradientSection}>
          ...
          <div style={(!radialVal && !conicVal && showCloseBtn) ? { paddingTop: '15px', ...styles.inputSection } : styles.inputSection} >
            <div style={{ ...styles.gradientSectionInputDiv, ...styles.outerDivStyle }}>
              {colorOne.map((e, i) => {
                return (<div style={selectedInput === i + 1 ? { ...styles.outerDivStyle, border: '2px solid ' + e } : {}} key={i} >
                  <div style={selectedInput !== i + 1 ? { ...styles.outerDivStyle, border: '2px solid ' + e, background: e, ...styles.inputSectionDiv } : { ...styles.outerDivStyle, border: '2px solid ' + e, background: e, margin: '3px' }} onClick={() => { setSelected(e); setSelectedInput(i + 1) }}></div></div>)
              })}
              {colorOne.length < 4 && <span><HiPlusCircle style={{ ...styles.inputSectionSpanSvg, ...styles.inputSectionDiv }} onClick={() => {
                setColorOne((current => [...current, '#000000'])); setTimeout(() => {
